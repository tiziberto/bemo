...
interface ProfDto {
  ...
  userId: number | null; activo: boolean; esDerivaor: boolean;
}
...
const condicionesIva = ref<{ id: number; descripcion: string }[]>([]);

const showNewForm = ref(false);
const newFormDerivadorMode = ref(false);
const newForm = ref({ nombre: "", dni: "", matricula: "", telefono: "", email: "", condicionIvaId: null as number | null, especialidadesIds: [] as number[], obrasSocialesIds: [] as number[], estudiosIds: [] as number[], sucursalesIds: [] as number[], esDerivaor: false });
const newSaving = ref(false);
const newError = ref("");

const editingId = ref<number | null>(null);
const editForm = ref({ nombre: "", dni: "", matricula: "", telefono: "", email: "", condicionIvaId: null as number | null, especialidadesIds: [] as number[], obrasSocialesIds: [] as number[], estudiosIds: [] as number[], sucursalesIds: [] as number[] });
...
async function loadPage(page: number) {
  ...
  try {
    ...
    const promises: Promise<any>[] = [
      axios.get(`${API}/profesionales?${params}`, { headers: headers() }),
    ];
    if (page === 0) {
      promises.push(
        axios.get(`${API}/especialidades`, { headers: headers() }),
        axios.get(`${API}/obras-sociales?size=1000`, { headers: headers() }),
        axios.get(`${API}/estudios?size=1000`, { headers: headers() }),
        axios.get(`${API}/sucursales`, { headers: headers() }),
        axios.get(`${API}/condiciones-iva`, { headers: headers() }),
      );
    }
    const [profRes, espRes, osRes, estRes, sucRes, civRes] = await Promise.all(promises);
    const pageData = profRes.data;
    profesionales.value = pageData.content;
    totalPages.value = pageData.totalPages;
    totalElements.value = pageData.totalElements;
    if (espRes) especialidades.value = espRes.data;
    if (osRes) obrasSociales.value = osRes.data.content ?? osRes.data;
    if (estRes) estudios.value = estRes.data.content ?? estRes.data;
    if (sucRes) sucursales.value = sucRes.data;
    if (civRes) condicionesIva.value = civRes.data;
...
async function createProf() {
  newError.value = "";
  if (!newForm.value.nombre) { newError.value = "Nombre es obligatorio."; return; }
  ...
  if (!newForm.value.condicionIvaId) { newError.value = "Condición IVA es obligatoria."; return; }
  ...
  try {
    ...
    newForm.value = { nombre: "", dni: "", matricula: "", telefono: "", email: "", condicionIvaId: null, especialidadesIds: [], obrasSocialesIds: [], estudiosIds: [], sucursalesIds: [], esDerivaor: false };
    showNewForm.value = false;
    newFormDerivadorMode.value = false;
    await loadData();
  } catch (e: any) {
...
function startEdit(p: ProfDto) {
  ...
  editForm.value = {
    nombre: p.nombre, dni: p.dni ?? "", matricula: p.matricula ?? "",
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <button v-if="isAdmin" @click="showNewForm = !showNewForm; newFormDerivadorMode = false; newForm.esDerivaor = false"
          ...
          <component :is="showNewForm && !newFormDerivadorMode ? X : Plus" class="h-4 w-4" />
          {{ showNewForm && !newFormDerivadorMode ? "Cancelar" : "Nuevo Profesional" }}
        </button>
        <button v-if="isAdminOrRecepcion"
          @click="showNewForm = !showNewForm; newFormDerivadorMode = true; newForm.esDerivaor = true"
          class="inline-flex items-center gap-2 rounded-md border border-blue-400 bg-blue-50 px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-100 transition-colors">
          <component :is="showNewForm && newFormDerivadorMode ? X : Plus" class="h-4 w-4" />
          {{ showNewForm && newFormDerivadorMode ? "Cancelar" : "Nuevo Derivador" }}
        </button>
      </div>
    </div>
    ...
    <div class="flex gap-2">
      <div class="relative flex-1">
        ...
        <Input v-model="searchQuery" placeholder="Buscar por nombre o matrícula..." class="pl-9" @keyup.enter="loadData" />
      </div>
      <Button variant="outline" @click="loadData">Buscar</Button>
    </div>
    ...
    <Card v-if="showNewForm">
      <CardHeader>
        <CardTitle class="text-base flex items-center gap-2">
          ...
          <span v-if="newFormDerivadorMode"
      ...
      <CardContent>
        <form @submit.prevent="createProf" class="space-y-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="space-y-1">
              <label class="text-sm font-medium">Condición IVA <span class="text-destructive">*</span></label>
              <Combobox v-model="newForm.condicionIvaId" :options="condicionesIva.map(c => ({ value: String(c.id), label: c.descripcion }))" placeholder="Seleccione..." />
            </div>
          </div>
    ...
    <Card>
      <CardContent class="p-0">
        ...
        <div v-else-if="profesionales.length === 0" class="p-6 text-center text-muted-foreground">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <template v-for="p in profesionales" :key="p.id">
                <!-- Normal row -->
                <tr v-if="editingId !== p.id" class="border-b hover:bg-muted/30 transition-colors">
                  <td class="px-4 py-3">
                    <div class="font-medium flex items-center gap-2">
                      {{ p.nombre }}
                      <span v-if="p.esDerivaor"
                        class="text-xs px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium border border-blue-200">
                        Derivador
                      </span>
                    </div>
