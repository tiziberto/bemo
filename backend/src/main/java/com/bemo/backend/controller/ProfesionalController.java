...
public class ProfesionalController {
    ...
    public ResponseEntity<?> getAll(
        ...
        return ResponseEntity.ok(service.getPage(q, page, size));
    }
    ...
    public ResponseEntity<?> create(@RequestBody ProfesionalDto dto, Authentication auth) {
        try {
            boolean isRecepcion = auth.getAuthorities().stream()
                    .anyMatch(a -> a.getAuthority().equals("ROLE_RECEPCION"));
            if (isRecepcion && !Boolean.TRUE.equals(dto.getEsDerivaor())) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Recepción solo puede crear profesionales derivadores"));
            }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'RECEPCION')")
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody ProfesionalDto dto, Authentication auth) {
        try {
            boolean isRecepcion = auth.getAuthorities().stream()
                    .anyMatch(a -> a.getAuthority().equals("ROLE_RECEPCION"));
            if (isRecepcion && !Boolean.TRUE.equals(dto.getEsDerivaor())) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Recepción solo puede modificar profesionales derivadores"));
            }
